...
class DailyAttendanceStudentRemark(models.TransientModel):
    ...
    academic_year_id = fields.Many2one("academic.year", ondelete="restrict",
                                       string="Academic Session",
                                       default=lambda obj: obj.env['academic.year'
                                                                   ].search([('current', '=', True)]))
    course_id = fields.Many2one("school.standard", "Semesters",
                                ondelete="restrict")
    ...
    subject_ids = fields.Many2many("subject.subject", 'subject_wizard_rel',
                                   'subject_id', 'wizard_id', "Subject",
                                   ondelete="restrict")
    ...
    def _send_monthly_attendance(self):
        pr_mon = pre_month = (
            date.today().replace(day=1) - timedelta(days=1)).month
        pr_mon = str(pr_mon)
        for subject in self.env['subject.subject'].search([]):
            for user in subject.teacher_ids:
                ...
                academic_year = self.env['academic.year'].search([
                    ('current', '=', True)])
                last_day_month = calendar.monthrange(int(academic_year.code),
                                                     int(pre_month))[1]
                start_date_str = (str(int(academic_year.code)) + '-' +
                                  str(int(pre_month)) + '-01')
                end_date_str = (str(int(academic_year.code)) + '-' +
                                  str(int(pre_month)
                                      ) + '-' + str(last_day_month) + ' 23:00:00')
                self._cr.execute('''select id
                                    from daily_attendance WHERE
                                    state = 'validate' and
                                    standard_id = %s and
                ...
                template_id = self.env['ir.model.data'].get_object_reference(
                    'school_attendance',
                    'email_template_monthly_attendace')[1]
    ...
    def get_total_class(self, rec, teacher, subject):
        total_class = 0
        user = self.env['school.teacher'].search([('name', '=', teacher)
                                                  ], limit=1)
        subject = self.env['subject.subject'].search([('id', '=', subject)],
                                                     limit=1)
        last_day_month = calendar.monthrange(int(rec.academic_year_id.code),
                                             int(rec.month))[1]
        start_date_str = (str(int(rec.academic_year_id.code)) + '-' +
    ...
    def print_report(self):
        ...
        for rec in self:
            ...
            for att in self.env['daily.attendance'].browse(records):
                ...
                if not group_data:
                    group_data.append({'user': att.user_id,
                                        ...
                                        if day_date in matched_dates or not matched_dates:
                                            if att_data.get(student.stud_id.name) and att_data.get(student.stud_id.name).get(att_count):
                                                if att_data.get(student.stud_id.name).get(att_count) != 'A':
                                                    status = int(att_data.get(student.stud_id.name).get(
                                                        att_count)) + no_of_class
                                    else:
                                        if day_date in matched_dates:
                                            if att_data.get(student.stud_id.name) and att_data.get(student.stud_id.name).get(att_count):
                                                if att_data.get(student.stud_id.name).get(att_count) != 'A':
                                                    status = int(att_data.get(
                                                        student.stud_id.name).get(att_count))
                                    if not att_data.get(student.stud_id.name):
                                        ...
                                        for stu in data:
                                            if stu.get('name') == student.stud_id.name:
                                                if not student.is_present:
                                                    stu.update({
                                                        'total_absent': stu.get('total_absent') + no_of_class})
                                                stu.get('att').update(
                                                    {att_count: str(status)})
                                else:
                                    status = ''
            ...
            for data in res_data:
                ...
                # Main Header
                sheet.merge_range(0, 0, 0, len(month_days) + 4,
                ...
                # Sub Headers
                sheet.merge_range(1, 0, 1, 8,
                                  'Name of the Teacher:' +
                                  str(data.get('user')),
                                  head_fmt_left)
                sheet.merge_range(1, 9, 1, 19,
                                  'Month:' + str(months.get(rec.month)) +
                ...
                for line in data.get('result_data'):
                    ...
                    if line.get('divisions') or data.get('elective'):
                        ...
                        sheet.write(row, col, line.get(
                            'student_code'), tbl_data_fmt)
                    sheet.write(row, col + 1, line.get('name'),
                                tbl_data_fmt_left)
                    sheet.write(
                        row, col + 2, line.get('stud_reg_code'), tbl_data_fmt_left)
                    ...
                    for date in month_days:
                        if line.get('att').get(date):
                            if line.get('att').get(date) not in ['A', False]:
                                present_no = present + \
                                    int(line.get('att').get(date))
                                present = present + \
                                    int(line.get('att').get(date))
                            if line.get('att').get(date) == 'A':
                                absent += 1
                                sheet.write(row, col, line.get(
                                    'att').get(date), tbl_data_fmt)
                            elif line.get('att').get(date) != 'A':
                                sheet.write(row, col, present_no,
                                            tbl_data_fmt_p)
                        col += 1
                    sheet.write(row, col, present, tbl_data_fmt_p)
                    absent_res = absent
                    self._cr.execute('''select id, medium_id
                                            from student_student WHERE
                                            roll_no = %s
                                        ''' % (line.get('roll_no')))
                    student = self._cr.fetchone()
                    if student:
                        total_class_att = self.get_total_class(rec, data.get('user'),
                                                               data.get('subject_id'))[1]
                        if not total_class_att.get('total'):
                            if student[1] == 3:
                                if present + absent != total_class_att.get('A'):
                                    absent_res = total_class_att.get(
                                        'A') - present
                            elif student[1] == 4:
                    ...
                    if total_class_att.get('A') == 0:
                        ...
                        sheet.write(
                            row, col + 1, line.get('total_absent'), tbl_data_fmt)
                    row += 1
                    count += 1
            ...
            doc_id = attch_obj.create({
                'name': str(months.get(rec.month)) + ' ' + str(rec.course_id.name) + ' ' + 'Monthly Attendance.xlsx',
                'datas': data,
                'res_model': 'monthly.attendance.wizard',
            })

...
class AttendanceSheet(models.Model):
    ...
    attendance_ids = fields.One2many('attendance.sheet.line', 'sheet_id',
                                     'Attendance')
        ...
        if self.standard_id:
            ...
            for stud_id in stud_ids:
                student_list.append({'roll_no': stud_id.roll_no,
                                     'name': stud_id.name})
        self.attendance_ids = [(0, 0, res) for res in student_list]
...
class DailyAttendance(models.Model):
    ...
    def _total(self):
        self.total_student = len(self.student_ids)

    @api.one
    @api.depends('student_ids.is_present')
    def _present(self):
        count = 0
        if self.student_ids:
    ...
    @api.one
    @api.depends('total_student', 'total_presence')
    def _absent(self):
        self.total_absent = self.total_student - self.total_presence

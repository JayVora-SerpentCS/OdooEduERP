...
class attendance_sheet(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        res = super(attendance_sheet, self).fields_view_get(view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
        if context is None:
            context = {}
...
class daily_attendance(models.Model):
    ...
    date = fields.Date("Today's Date", default=lambda *a: time.strftime('%Y-%m-%d')) 
    ...
    state = fields.Selection([('draft', 'Draft'), ('validate', 'Validate')], 'State', readonly=True, default='draft')
    total_student = fields.Integer(compute="_total", method=True, store=True, string='Total Students')
    ...
    def onchange_standard_id(self, standard_id):
        ...
        stud_obj = self.env['student.student']
        if standard_id:
            self._cr.execute("""select id from student_student where standard_id=%s""",(standard_id,))
    ...
    def attendance_draft(self):
        ...
        for daily_attendance_data in self:
            ...
            year_search_ids = academic_year_obj.search([('code', '=', date.year)])
            month_search_ids = academic_month_obj.search([('code', '=', date.month)])
            for line in daily_attendance_data.student_ids:
                line.write({'is_present': False, 'is_absent': False})
                
            attendance_sheet_domain = [('standard_id', '=', daily_attendance_data.standard_id.id), ('month_id', '=', month_search_ids.id), ('year_id', '=', year_search_ids.id)]
    ...
    def attendance_validate(self):
        ...
        for line in self:
            ...
            year = date.year
            year_ids= acadmic_year_obj.search([('date_start', '<=', date), ('date_stop', '>=', date)])
            month_ids= acadmic_month_obj.search([('date_start', '<=', date), ('date_stop', '>=', date), ('year_id', 'in', year_ids.ids)])
            if month_ids:
                month_data = month_ids
                att_sheet_ids = attendance_sheet_obj.search([('month_id', 'in', month_ids.ids), ('year_id', 'in', year_ids.ids)])

...
class SchoolEventParameter(models.Model):
    ...
    name = fields.Char(
        "Parameter Name", required=True, help="Add event parameter"
    )
...
class SchoolEventParticipant(models.Model):
    ...
    score = fields.Float("Score", default=0, help="Score obtained by student")
    event_id = fields.Many2one(
        "school.event", "Event", readonly=True, help="Name of event"
    )
    stu_pid = fields.Char(
    ...
    win_parameter_id = fields.Many2one(
    ...
    rank = fields.Integer(
        "Rank",
        help="""The sequence field is used to Give
...
class SchoolEvent(models.Model):
    ...
    start_reg_date = fields.Date(
        "Start Registration Date", help="Event registration starting date"
    )
    last_reg_date = fields.Date(
        "Last Registration Date", help="Last Date of registration"
    )
    contact_per_id = fields.Many2one(
        "hr.employee", "Contact Person", help="Event contact person"
    )
    supervisor_id = fields.Many2one(
        "hr.employee", "Supervisor", help="Event Supervisor Name"
    )
    parameter_id = fields.Many2one(
        "school.event.parameter",
        "Parameter",
        help="Parameters of the Event like (Goal, Point)",
    )
    maximum_participants = fields.Integer(
        "Maximum Participants", help="Maximum Participant of the Event"
    )
    ...
    code = fields.Many2one(
        "school.school", "Organizer School", help="Event Organized School"
    )
    ...
    color = fields.Integer("Color Index", default=0, help="Choose color index")
    ...
    @api.constrains(
        "start_date", "end_date", "start_reg_date", "last_reg_date"
    )
...
"""
    ...
    def create(self, vals):
        ...
        st_lst = []
        for student in res.part_standard_ids.mapped("student_ids"):
...
class SchoolEventRegistration(models.Model):
    ...
    is_holiday = fields.Boolean(
        "Is Holiday(s)", help="Checked if the event is organized on holiday."
    )
    ...
    def check_student_registration(self):
        """Inherited unlink method to check the state at the record deletion"""
        student_event = self.search(
            [
                ("part_name_id", "=", self.part_name_id.id),
                ("id", "not in", self.ids),
                ("state", "=", "confirm"),
                ("name", "=", self.name.id),
            ]
    ...
    def regi_confirm(self):
        ...
        for rec in self:
            participants = rec.name.participants + 1
...
Last Registration date is over for this Event!"""
            ...
            vals = {
                "stu_pid": str(rec.part_name_id.pid),

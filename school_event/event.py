...
class school_event(models.Model):
    ...
    participants = fields.Integer(compute='_participants', string='Participants', readonly=True)
...
class school_event_registration(models.Model):
    ...
    def regi_cancel(self):
        ...
        for reg_data in self:
            ...
            event_reg_id.write({'part_ids':[(6,0,list1)], 'participants': participants})
        return True
    ...
    def regi_confirm(self):
        ...
        for reg_data in self:
            ...
            if participants > event_data.maximum_participants:
                raise except_orm(_('Error !'),_('Participation in this Event is Full.'))
            
            #check last registration date is over or not
            if reg_data.reg_date > event_data.last_reg_date:
                raise except_orm(_('Error !'),_('Last Registration date is over for this Event.'))

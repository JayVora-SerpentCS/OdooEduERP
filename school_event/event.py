...
class school_event(models.Model):
    ...
    event_type = fields.Selection([('intra', 'Intra School'),
                                   ('inter', 'Inter School')], 'Event Type',
                                  help='Event is either Intra \
    ...
    part_ids = fields.Many2many('school.event.participant',
                                ...
                                order_by='score')
    code = fields.Many2one('school.school', 'Organiser School',
                           help='Event Organised School')
    ...
    def _check_all_dates(self):
        if self.start_date and self.end_date and \
                self.start_reg_date and self.last_reg_date:
    ...
    def regi_confirm(self):
        ...
        for reg_data in self:
            ...
            if participants > event_data.maximum_participants:
                raise except_orm(_('Error !'), _('Participation in this \
                                                  Event is Full.'))

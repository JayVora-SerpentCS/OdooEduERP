...
class student_fees_register(models.Model):
    ...
    period_id = fields.Many2one('account.period', 'Force Period',
                                ...
                                'done')], states={'confirm': [('readonly',
                                True)]}, help="Keep empty to use the \
                                period of the validation(invoice) date.")
    company_id = fields.Many2one('res.company', 'Company', required=True,
                                 change_default=True, readonly=True,
                                 states={'draft': [('readonly', False)]},
                                 default=lambda self, cr, uid, c:
                ...
                if old_slips:
                    ...
                    for sid in old_slips:
                        workflow.trg_validate(self._uid, 'student.payslip',
                                       sid.id, 'payslip_confirm', self._cr)
                else:
                    res = {
                        'student_id': stu.id,
                        'register_id': vals.id,
                    ...
                    workflow.trg_validate(self._uid, 'student.payslip',
                               slip_id.id, 'payslip_confirm', self._cr)
            amount = 0
...
class student_fees_structure(models.Model):
    ...
    _sql_constraints = [
                        ('code_uniq', 'unique(code)', 'The code of the Fees \
...
class student_payslip(models.Model):
    ...
    fees_structure_id = fields.Many2one('student.fees.structure', 'Fees \
                                         Structure', states={'paid':
                                         [('readonly', True)]})
    ...
    student_id = fields.Many2one('student.student', 'Student', required=True,
                                       states={'paid': [('readonly', True)]})
    date = fields.Date('Date', readonly=True, default=lambda * a:
                                        time.strftime('%Y-%m-%d'))

...
class student_transports(models.Model):
    ...
    total_participantes = fields.Integer(compute='_total_participantes', method=True, string='Total Participantes', readonly=True)
    ...
    def trans_regi_confirm(self):
        ...
        for reg_data in self:
            ...
            if reg_data.for_month <= 0:
                raise Warning(_('Error !'), _('Sorry Registration months must one or more then one.'))
            # First Check Is there vacancy or not
            person = int(reg_data.vehicle_id.participant) + 1
            if reg_data.vehicle_id.capacity < person:
                raise Warning(_('Error !'), _('There is No More vacancy on this vehicle.'))

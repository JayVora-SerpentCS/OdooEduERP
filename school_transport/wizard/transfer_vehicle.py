...
class TransferVehicle(models.TransientModel):
    ...
    old_vehicle_id = fields.Many2one("fleet.vehicle", "Old Vehicle No",
        required=True)
    new_vehicle_id = fields.Many2one("fleet.vehicle", "New Vehicle No",
        required=True)
    ...
    def vehicle_transfer(self):
        ...
        for rec in self:
            ...
            if vehi_data.id == vehi_new_data.id:
                raise UserError(_(
                    "Error ! Sorry you can not transfer in same vehicle."))
            # First Check Is there vacancy or not
            person = int(vehi_data.participant) + 1
            if vehi_data.capacity < person:
                raise UserError(_(
                    "Error ! There is No More vacancy on this vehicle."))
            # remove entry of participant in old vehicle.
            participants = [
                prt_id.id for prt_id in vehi_data.vehi_participants_ids
            ]
            ...
            participants = [
                prt_id.id for prt_id in vehi_new_data.vehi_participants_ids]
            participants.append(rec.participation_id.id)
            vehi_new_data.write(
                {"vehi_participants_ids": [(6, 0, participants)]})

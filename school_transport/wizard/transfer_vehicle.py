...
class transfer_vehicle(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if self._context.get('active_id'):
            vehicle_id = self._context.get('active_id')
            student = self.env['student.student'].browse(vehicle_id)
            if 'name' in fields:
    ...
    def vehicle_transfer(self):
        ...
        for new_data in self.browse(self.id):
            ...
            if new_data.old_vehicle_id.id == new_data.new_vehicle_id.id:
                raise except_orm(_('Error !'), _('Sorry you can not transfer \
                                   in same vehicle.'))
            # First Check Is there vacancy or not
            person = int(vehi_data.participant) + 1
            if vehi_data.capacity < person:
                raise except_orm(_('Error !'), _('There is No More \
                                    vacancy on this vehicle.'))
...
#            {'vehi_participants_ids':[(6,0,participants)]}, context=context)
            ...
            old_veh_id.write({'vehi_participants_ids': [(6, 0,
                                                         participants)]})
...
#            {'vehi_participants_ids':[(6,0,participants)]}, context=context)
            ...
            new_veh_id.write({'vehi_participants_ids': [(6, 0,
                                                         participants)]})

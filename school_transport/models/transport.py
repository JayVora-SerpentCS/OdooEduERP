...
class HrEmployee(models.Model):
    ...
    def check_licence_number(self):
        ...
        driver_rec = self.search([('licence_no', '=', self.licence_no),
                              ('id', 'not in', self.ids)])
        if driver_rec:
...
class TransportParticipant(models.Model):
    ...
    active = fields.Boolean('Active', default=True, help='''Marked transport
...
class TransportRegistration(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'),
                             ...
                             default='draft', help='''State of the transport
                             registration form''')
    vehicle_id = fields.Many2one('transport.vehicle', 'Vehicle No',
                                 required=True, help='Enter transport vehicle')
    point_id = fields.Many2one('transport.point', 'Point', required=True,
...
For this much Months Registration is not Possible because Root 
            ...
            vehicle_rec.sudo().write({'vehi_participants_ids':
            ...
            part_name_rec.sudo().write({'transport_ids':
                                       [(6, 0, transport_list)]})
            # make entry in transport.
            ...
            stu_tran_rec.sudo().write({'trans_participants_ids':

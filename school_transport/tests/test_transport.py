...
class TestTransport(common.TransactionCase):
    def setUp(self):
        ...
        #       Create Driver
        self.driver = self.transport_driver_obj.create(
            {
                "name": "Driver test",
        ...
        self.transport_vehicle = self.transport_vehicle_obj.create(
            {
        ...
        self.transport_point = self.transport_point_obj.create(
            {"name": "test point-1", "amount": "1000"}
        )
        ...
        self.transfer_vehicle = self.transfer_vehicle_obj.create(
            {
                "name": self.student.id,
    ...
    def test_transport(self):
        ...
        self.assertIn(
            self.transport_root.trans_vehicle_ids, self.transport_vehicle
        )
        self.assertIn(
            self.transport_root.trans_point_ids, self.transport_point
        )

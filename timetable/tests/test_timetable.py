...
class TestTimetableLine(common.TransactionCase):

    def setUp(self):
        super(TestTimetableLine, self).setUp()
    ...
    def test_check_room(self):
        with self.assertRaises(UserError), self.cr.savepoint():
            ...
            self.timetable_rec = self.env['time.table'].search([('id', '!=',
                                                                 self.time_table.id)])
            print("tt rec==============", self.timetable_rec)

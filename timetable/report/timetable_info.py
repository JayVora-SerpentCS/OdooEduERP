...
class timetable(report_sxw.rml_parse):
    ...
    def _get_timetable(self, timetable_id):
        ...
        "t.teacher_id= hr.id  and hr.resource_id = r.id  and"
        "table_id = %d group by start_time,end_time,s.name,week_day,"
        "r.name order by start_time" % (timetable_id.id))
        res = self.cr.dictfetchall()
        # self.cr.execute("select start_time,end_time from time_table_line where
        # table_id=%d group by start_time,end_time  order by start_time"%
        # (timetable_id.id))

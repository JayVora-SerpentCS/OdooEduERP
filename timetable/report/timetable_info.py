...
class timetable(report_sxw.rml_parse):
    ...
    def _get_timetable(self, timetable_id):
        ...
        self.cr.execute("select t.start_time,t.end_time,s.name,week_day,r.name"
                        "as teacher from time_table_line t, subject_subjects,"
                        "resource_resource r, hr_employee hr where"
                        "t.subject_id= s.id and t.teacher_id= hr.id  and "
                        "hr.resource_id = r.id  and table_id = %d group by"
                        "start_time,end_time,s.name,week_day,r.name order by"

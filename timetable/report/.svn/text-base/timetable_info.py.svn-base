...
class timetable(report_sxw.rml_parse):
    ...
    def _get_timetable(self, timetable_id):
        ...
        self.cr.execute("select t.start_time,t.end_time,s.name,week_day,r.name as teacher from time_table_line t, subject_subject s, resource_resource r, hr_employee hr where t.subject_id= s.id and t.teacher_id= hr.id  and hr.resource_id = r.id  and table_id = %d group by start_time,end_time,s.name,week_day,r.name order by start_time"%(timetable_id.id))
        res = self.cr.dictfetchall()
#        self.cr.execute("select start_time,end_time from time_table_line where table_id=%d group by start_time,end_time  order by start_time"%(timetable_id.id))
        self.cr.execute("select start_time,end_time from time_table_line where table_id=%d group by start_time,end_time  order by start_time"%(timetable_id.id))
...
report_sxw.report_sxw('report.time.table', 'time.table', 'addons/timetable/report/timetable.rml', parser=timetable, header="external")

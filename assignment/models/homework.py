...
class SchoolTeacherAssignment(models.Model):
    ...
    student_assign_ids = fields.One2many("school.student.assignment",
        "teacher_assignment_id", string="Student Assignments",
        help="Enter student assignments")
    type_submission = fields.Selection([("hardcopy", "Hardcopy(Paperwork)"),
        ("softcopy", "Softcopy")], default="hardcopy",
        string="Submission Type", help="Select Assignment type")
...
class SchoolStudentAssignment(models.Model):
    ...
    teacher_assignment_id = fields.Many2one("school.teacher.assignment",
        string="Teachers", help="Teacher assigments")
    student_standard = fields.Many2one("standard.standard",
        "Student Standard", help="Select student standard")
...
"""Kindly attach file with format: %s!""")% self.attachfile_format)
    ...
    def done_assignment(self):
        ...
        if self.submission_type == "softcopy" and not self.submit_assign:
            raise ValidationError(_(
"""You have not attached the homework! Please attach the homework!"""))
        self.state = "done"

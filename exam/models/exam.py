            ...
            for rec in self:
                if rec.table_id.timetable_type == 'exam':
                    if not rec.teacher_id:
                        raise ValidationError(_('''Enter Supervisior'''))
        ...
        for rec in self.table_id.exam_timetable_line_ids:
            ...
            if rec.id not in self.ids:
                if (record.timetable_type == 'exam' and
                    ...
                    raise ValidationError(_('''Already Exam at
                                            same Date and Time'''))
                if (record.timetable_type == 'exam' and
                        self.table_id.timetable_type == 'exam' and
...
class AdditionalExam(models.Model):
    ...
    def check_marks(self):
        if self.minimum_marks > self.maximum_marks:
            raise ValidationError(_('''Maximum marks should be greater
                                    than minimum marks'''))

...
class StudentStudent(models.Model):
    ...
    exam_results_ids = fields.One2many("exam.result", "student_id",
        "Exam History", readonly=True, help="Student exam history")
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False,
        ...
        return super(StudentStudent, self)._search( args=args, offset=offset,
...
class ExtendedTimeTable(models.Model):
    ...
    timetable_type = fields.Selection(selection_add=[("exam", "Exam")],
        ...
        ondelete={"exam": "set default"}, help="Select timetable type")
    exam_timetable_line_ids = fields.One2many("time.table.line", "table_id",
        "TimeTable Lines", help="Timetable")
...
"You cannot delete schedule of exam which is in running!"))
    ...
    def _check_exam(self):
        ...
        if self.timetable_type == "exam":
            ...
            for rec in line_ids:
                records = [rec_check.id for rec_check in line_ids if (
...
"You cannot set exam at same time %s  at same day %s for teacher %s!")
                % (rec.start_time, rec.day_of_week, rec.teacher_id.name))
...
_("Invalid Date Error! Exam Date should be greater than today!"))
                ...
                if line.timetable_id:
                    for tt in line.timetable_id.exam_timetable_line_ids:
                        if not rec.start_date <= tt.exm_date <= rec.end_date:
                            raise ValidationError(_(
"Invalid Exam Schedule! Exam Dates must be in between Start date and End date!"))
        ...
        if not self.active:
            for result in result_obj.search([("s_exam_ids", "=", self.id)]):
                if result.state != "done":
                    raise ValidationError(_(
"Kindly,mark as done %s examination results!") % (self.name))
    ...
    standard_id = fields.Many2many("standard.standard",
        "standard_standard_exam_rel", "standard_id", "event_id",
        "Participant Standards", help="Select Standard")
    ...
    state = fields.Selection([("draft", "Draft"), ("running", "Running"),
        ("finished", "Finished"), ("cancelled", "Cancelled")], "State",
        readonly=True, default="draft", help="State of the exam")
    ...
    exam_schedule_ids = fields.One2many("exam.schedule.line",
        "exam_id", "Exam Schedule", help="Enter exam schedule")
...
class ExamScheduleLine(models.Model):
    ...
    standard_ids = fields.Many2many("standard.standard",
        string="Participant Standards", help="Enter standards for the exams")
...
class AdditionalExam(models.Model):
    ...
    color_name = fields.Integer("Color index of creator",
        compute="_compute_color_name", store=False, help="Select color")

    @api.model
    def create(self, vals):
        vals['addtional_exam_code'] = self.env['ir.sequence'
                            ].next_by_code('additional.exam') or _('New')
...
class ExamResult(models.Model):
    ...
    def _update_rollno_standard(self, student):
        ...
        return {"roll_no": student_rec.roll_no,
        ...
        for rec in self:
            for line in rec.result_ids:
                if line.maximum_marks == 0:
                    ...
                    raise ValidationError(_(
    'Kindly add maximum marks of subject "%s".')% (line.subject_id.name))
                elif line.minimum_marks == 0:
                    raise ValidationError(_(
    'Kindly add minimum marks of subject "%s".')% (line.subject_id.name))
                elif (line.maximum_marks == 0 or
                        line.minimum_marks == 0) and line.obtain_marks:
                    raise ValidationError(_(
    'Kindly add marks details of subject "%s"!')% (line.subject_id.name))
            rec.write({"grade": rec.grade,
...
class AdditionalExamResult(models.Model):
    ...
    def _validate_obtain_marks(self):
        ...
        if self.obtain_marks > self.a_exam_id.maximum_marks:
            raise ValidationError(
                _(
                    """Obtained marks should not extend maximum marks!"""
                )
            )

...
class ProductProduct(models.Model):
    ...
    def _default_categ(self):
        ...
        res = False
        try:
            res = self.env.ref("school_library_management.product_category_1").id
        except ValueError:
            res = False
        return res
    ...
    def action_book_req(self):
        ...
        for rec in self:
            ...
            action = self.env.ref("school_library_management.action_lib_book_req")
            result = action.read()[0]
            req = [request_rec.id for request_rec in book_req]
            if len(req) != 1:
                result["domain"] = "[('id', 'in', " + str(req) + ")]"
            else:
                res = self.env.ref("school_library_management.view_book_library_req_form", False)
                result["views"] = [(res and res.id or False, "form")]

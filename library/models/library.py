...
class LibraryCard(models.Model):
    ...
    def on_change_student(self):
        ...
        student_rec = self.env["student.student"].browse(self.student_id.id)
...
class LibraryBookIssue(models.Model):
        ...
        for rec in self:
            if rec.card_id:
                ...
                if rec.state == "issue" or rec.state == "reissue":
                    if rec.card_id.book_limit > card_ids - 1:
                        return True
                    else:
                    ...
                    if rec.card_id.book_limit > card_ids:
                        return True
                    else:
                        raise UserError(
                            _(
    ...
    def issue_book(self):
        ...
        if self.card_id.end_date < curr_dt and self.card_id.end_date > curr_dt:
            raise UserError(
                _(
                    """The Membership of library \
card is over!"""
                )
            )
        code_issue = self.env["ir.sequence"].next_by_code(
            "library.book.issue"
        ) or _("New")
        for rec in self:
...
class LibraryBookRequest(models.Model):
    ...
    active = fields.Boolean(
        default=True,
        help="""Set active to false to hide
    ...
    def check_book_request(self):
        ...
        if book_request_rec:
            raise UserError(
                _(
                    """
 You cannot request same book on same card number more than once at same time!
                            """
                )
            )
    ...
    def confirm_book_request(self):
        ...
        if (
            ...
            raise UserError(
                _(
                    """The Membership of library card is over!
            """
                )
            )

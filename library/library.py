...
class library_card(models.Model):
    ...
    book_limit = fields.Integer('No of Books Limit on Card', required=True)
    ...
    standard_id = fields.Many2one('standard.standard', related='student_id.standard_id', string='Standard', readonly=True)
    gt_name = fields.Char(compute="get_name", method=True, string='Name')
    user = fields.Selection([('student', 'Student'), ('teacher', 'Teacher')],
                            "User")
    roll_no = fields.Integer(related='student_id.roll_no', string='Roll No', readonly=True)
    teacher_id = fields.Many2one('hr.employee', 'Teacher Name')

    @api.onchange('student_id')
    def onchange_student(self):
        if self.student_id:
            self.student_name = self.student_id.name + ' ' + self.student_id.middle + ' ' + self.student_id.last
            self.standard_id = self.student_id.standard_id and self.student_id.standard_id.id
            self.roll_no = self.student_id and self.student_id.roll_no
...
#         for issue in self.browse(cr, uid, ids, context = context):
    ...
    issue_code = fields.Char('Issue No.', readonly=True,
    ...
    standard_id = fields.Many2one('standard.standard', string='Class', related='student_id.standard_id', readonly=True)
    roll_no = fields.Integer(related='student_id.roll_no', string='Roll No', readonly=True)
...
# methode for the workflow#
    ...
    def issue_book(self):
        ...
        @return : True
        '''
        book_obj = self.env["product.product"]
        book_ids = book_obj.search([('id', '=', self.name.id)])
#         for issue in self.browse(cr, uid, ids, context = context):
        if self.card_id:
            card_ids = self.search([('card_id', '=', self.card_id.id),
                                    ('state', 'in', ['issue', 'reissue'])])
            ...
            book_ids.remain_qty = book_ids.available_qty - 1
        return True
...
class library_book_request(models.Model):
    ...
    bk_nm = fields.Char(compute="gt_bname", method=True, string='Name',
                        store=True)
    remain_qty = fields.Integer('Remain Qty')
    email = fields.Char('Email')
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirmed'), ('cancel', 'Cancelled')], 'State', default='draft')
    book_available = fields.Selection([
                                       ('available', 'Available'), 
                                       ('not_available', 'Not Available')], 
                                      'Book Availibility', 
                                      readonly=True, help="when you confirm the request it will show you if book is available or not")
    ...
    def action_confirm(self):
        ...
        for rec in self:
            book_ids = book_obj.search([('id', '=', rec.name.id)])
            if book_ids:
                book_ids.remain_qty = book_ids.remain_qty - 1
                rec.remain_qty = book_ids.remain_qty
                if rec.remain_qty > 0:
                    rec.book_available = 'available'
                    rec.state = 'confirm'
                else:
                    rec.book_available = 'not_available'
                    rec.state = 'cancel'
            if rec.type == 'new':
                group_lib_id = ir_model_data.get_object_reference('library', 'group_librarian')[1]
                if group_lib_id:
                    group = group_obj.browse(group_lib_id)
                    for user in group.users:
                        if user.email:
                            template_id = ir_model_data.get_object_reference('library', 'email_template_new_book_request')[1]
                            template = self.pool.get('email.template').browse(cr, uid, template_id)
                            user_rec = self.env['res.users'].browse(self._uid)
                            server = email_server_obj.search([])[0]
                            self.pool.get('email.template').write(cr, SUPERUSER_ID, [template.id], {'email_to': user.email, 'email_from': user_rec.email})
    ...
    def action_cancel(self):
        ...
        if book_ids:
            book_ids.remain_qty = book_ids.remain_qty + 1
            self.remain_qty = book_ids.remain_qty
            self.state = 'cancel'

...
class student_evaluation(models.Model):
    ...
    def get_record(self):
        ...
        for stu_eval_rec in self.browse(self.ids):
            ...
            for i in range(0, len(eval_list)):
                eval_line_obj.create({"stu_eval_id" : eval_list[i], "eval_id" : self.id})
        return True
    ...
    date = fields.Date('Evaluation Date',required=True,default=lambda * a: time.strftime('%Y-%m-%d'))
    ...
    total = fields.Float(compute='_compute_total_points', method=True, string="Total Points")
    state = fields.Selection([('draft','Draft'),('start','Start'),('finished','Finish'),('cancelled','Cancel')], 'State',readonly=True,default='draft')
    user_id = fields.Many2one('res.users','User',readonly=True,default=get_user)
